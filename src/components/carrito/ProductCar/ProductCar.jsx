...
const ProductCar = ({name, price, quantity, img, handleTotal, total, id}) => {
  const {formatterPeso, favorites, setFavorites, inCar, setInCar} = useContext(AppContext)
  ...
  return (
    <article className='productCar'>
        <figure onClick={()=> navigate(`/producto/${id}`)} className='productCar__img' style={{background: `url(${img})`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
        </figure>
        <div className='productCar__items'>
          <div className='productCar__head'>
            <h2 onClick={()=> navigate(`/producto/${id}`)} className='productCar__name'>{name ? name: ''}</h2>
            <FaRegTimesCircle onClick={handleDelete}/>
          </div>
            <div className='productCar__items-values'>
               <p className='productCar__itemscantidad'>{quantity ? `x${quantity}` :  <IoHeart style={{color: 'red'}}/> }</p>
